...
def arm_control(model, data):
    ...
    mujoco.mj_jac(model, data, jacp=J, jacr=None,
                  point=np.array([[x], [y], [0]]), body=model.body("tip").id)

    H = np.empty((model.nv, model.nv))
    mujoco.mj_fullM(model, H, data.qM)
    H[[(i, i) for i in range(model.nv)]] += 0.01
    H = H / model.body("upper arm").subtreemass

    xvel, yvel, _ = J @ data.qvel
    xe, ye = xt - x, yt - y
    task_force = Kp * np.array([xe, ye, 0]) - Kd * np.array([xvel, yvel, 0])
    f = J.T @ task_force
